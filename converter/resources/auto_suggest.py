...
def _matches(prompt_lower: str, keywords: list[str]) -> bool:
    return any(isinstance(kw, str) and kw.lower() in prompt_lower for kw in keywords)
...
def _haiku_classify(prompt: str, rules: list[dict], cfg: dict) -> dict | None:
    ...
    cmd = [
        ...
        json.dumps(schema),
        "--max-budget-usd",
        str(cfg.get("max_budget_usd", 0.005)),
    ]
    env = {**os.environ}
...
def main() -> int:
    ...
    if not matched:
        haiku_cfg = (
            config.get("haiku_classifier", {}) if isinstance(config, dict) else {}
        )
