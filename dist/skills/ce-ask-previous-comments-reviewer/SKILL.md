...
Dispatch the **ce-previous-comments-reviewer** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-previous-comments-reviewer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-previous-comments-reviewer.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   ```
   [ce-persona=ce-previous-comments-reviewer via=ce-ask-direct]

   You are operating as the ce-previous-comments-reviewer specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-previous-comments-reviewer.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-previous-comments-reviewer` not found in this plugin's manifest. The plugin
...
- Trace tag `[ce-persona=ce-previous-comments-reviewer via=ce-ask-direct]` is inline debug
