...
Dispatch the **ce-learnings-researcher** specialist as a sub-agent.

## Steps

1. Run `ce-lite-persona ce-learnings-researcher --body` via Bash. The resolver is on
...
2. Spawn an `Agent` with `subagent_type: "general-purpose"` and
   `description: "ce-learnings-researcher: <one-line task summary>"` so traces stay
   ...
   You are operating as the ce-learnings-researcher specialist. Manifest declares
