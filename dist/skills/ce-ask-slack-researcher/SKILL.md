...
Dispatch the **ce-slack-researcher** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-slack-researcher.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-slack-researcher.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   You are operating as the ce-slack-researcher specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-slack-researcher.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-slack-researcher` not found in this plugin's manifest. The plugin
