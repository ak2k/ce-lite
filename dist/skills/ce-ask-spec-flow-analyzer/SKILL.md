...
`references/agent-prompts/ce-spec-flow-analyzer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-spec-flow-analyzer.md`) for the
...
`<plugin-root>/references/agent-prompts/ce-spec-flow-analyzer.md` does not exist (upstream rename,
