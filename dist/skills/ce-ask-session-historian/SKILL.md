...
Dispatch the **ce-session-historian** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-session-historian.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-session-historian.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   You are operating as the ce-session-historian specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-session-historian.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-session-historian` not found in this plugin's manifest. The plugin
