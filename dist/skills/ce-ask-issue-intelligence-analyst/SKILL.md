...
Dispatch the **ce-issue-intelligence-analyst** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-issue-intelligence-analyst.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-issue-intelligence-analyst.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   You are operating as the ce-issue-intelligence-analyst specialist. Your role is defined
   in the prompt body above. Manifest declares this role uses
   tools=[Read, Grep, Glob, Bash, mcp__github__*] and model=inherit. Honour those constraints:
...
`<plugin-root>/references/agent-prompts/ce-issue-intelligence-analyst.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-issue-intelligence-analyst` not found in this plugin's manifest. The plugin
