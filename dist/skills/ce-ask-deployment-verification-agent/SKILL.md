...
Dispatch the **ce-deployment-verification-agent** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-deployment-verification-agent.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-deployment-verification-agent.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   You are operating as the ce-deployment-verification-agent specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-deployment-verification-agent.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-deployment-verification-agent` not found in this plugin's manifest. The plugin
