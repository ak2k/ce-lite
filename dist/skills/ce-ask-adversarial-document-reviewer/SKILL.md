...
description: "Use when the document has >5 requirements or implementation units, makes significant architectural decisions, covers high-stakes domains, or proposes new abstractions. Challenges premises, surfaces unstated assumptions, and stress-tests decisions rather than evaluating document quality."
---

<!-- Generated by ce-lite converter — do not edit. Regenerate via converter/generate_wrappers.py. -->

Dispatch the **ce-adversarial-document-reviewer** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-adversarial-document-reviewer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-adversarial-document-reviewer.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   ```
   [ce-persona=ce-adversarial-document-reviewer via=ce-ask-direct]

   You are operating as the ce-adversarial-document-reviewer specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-adversarial-document-reviewer.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-adversarial-document-reviewer` not found in this plugin's manifest. The plugin
...
- Trace tag `[ce-persona=ce-adversarial-document-reviewer via=ce-ask-direct]` is inline debug
