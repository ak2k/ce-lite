---
name: ce-ask-kieran-typescript-reviewer
description: "Use when the diff touches TypeScript code. Reviews changes with Kieran's strict bar for type safety, clarity, and maintainability."
---

<!-- Generated by ce-lite converter — do not edit. Regenerate via converter/generate_wrappers.py. -->

Dispatch the **ce-kieran-typescript-reviewer** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-kieran-typescript-reviewer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-kieran-typescript-reviewer.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   ```
   [ce-persona=ce-kieran-typescript-reviewer via=ce-ask-direct]

   You are operating as the ce-kieran-typescript-reviewer specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-kieran-typescript-reviewer.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-kieran-typescript-reviewer` not found in this plugin's manifest. The plugin
...
- Trace tag `[ce-persona=ce-kieran-typescript-reviewer via=ce-ask-direct]` is inline debug
