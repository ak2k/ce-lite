...
Dispatch the **ce-git-history-analyzer** specialist as a sub-agent.

## Steps

1. Run `ce-lite-persona ce-git-history-analyzer --body` via Bash. The resolver is on
...
2. Spawn an `Agent` with `subagent_type: "general-purpose"` and
   `description: "ce-git-history-analyzer: <one-line task summary>"` so traces stay
   ...
   You are operating as the ce-git-history-analyzer specialist. Manifest declares
