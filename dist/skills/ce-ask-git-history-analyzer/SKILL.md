---
name: ce-ask-git-history-analyzer
...
Dispatch the **ce-git-history-analyzer** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-git-history-analyzer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-git-history-analyzer.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   ```
   [ce-persona=ce-git-history-analyzer via=ce-ask-direct]

   You are operating as the ce-git-history-analyzer specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-git-history-analyzer.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-git-history-analyzer` not found in this plugin's manifest. The plugin
...
- Trace tag `[ce-persona=ce-git-history-analyzer via=ce-ask-direct]` is inline debug
