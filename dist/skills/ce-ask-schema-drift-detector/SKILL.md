...
Dispatch the **ce-schema-drift-detector** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-schema-drift-detector.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-schema-drift-detector.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   ```
   [ce-persona=ce-schema-drift-detector via=ce-ask-direct]

   You are operating as the ce-schema-drift-detector specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-schema-drift-detector.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-schema-drift-detector` not found in this plugin's manifest. The plugin
...
- Trace tag `[ce-persona=ce-schema-drift-detector via=ce-ask-direct]` is inline debug
