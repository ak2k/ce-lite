...
Dispatch the **ce-design-lens-reviewer** specialist as a sub-agent.

## Steps

1. Run `ce-lite-persona ce-design-lens-reviewer --body` via Bash. The resolver is on
...
2. Spawn an `Agent` with `subagent_type: "general-purpose"` and
   `description: "ce-design-lens-reviewer: <one-line task summary>"` so traces stay
   ...
   You are operating as the ce-design-lens-reviewer specialist. Manifest declares
