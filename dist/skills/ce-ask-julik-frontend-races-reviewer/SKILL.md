...
`references/agent-prompts/ce-julik-frontend-races-reviewer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-julik-frontend-races-reviewer.md`) for the
...
`<plugin-root>/references/agent-prompts/ce-julik-frontend-races-reviewer.md` does not exist (upstream rename,
