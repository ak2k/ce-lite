...
Dispatch the **ce-ankane-readme-writer** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-ankane-readme-writer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-ankane-readme-writer.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   You are operating as the ce-ankane-readme-writer specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-ankane-readme-writer.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-ankane-readme-writer` not found in this plugin's manifest. The plugin
