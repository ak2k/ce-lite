...
Dispatch the **ce-coherence-reviewer** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-coherence-reviewer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-coherence-reviewer.md`) for the
...
`<plugin-root>/references/agent-prompts/ce-coherence-reviewer.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-coherence-reviewer` not found in this plugin's manifest. The plugin
