...
Dispatch the **ce-data-integrity-guardian** specialist as a sub-agent.

## Steps

1. Run `ce-lite-persona ce-data-integrity-guardian --body` via Bash. The resolver is on
...
2. Spawn an `Agent` with `subagent_type: "general-purpose"` and
   `description: "ce-data-integrity-guardian: <one-line task summary>"` so traces stay
   ...
   You are operating as the ce-data-integrity-guardian specialist. Manifest declares
