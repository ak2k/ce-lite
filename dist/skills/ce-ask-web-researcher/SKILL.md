...
`references/agent-prompts/ce-web-researcher.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-web-researcher.md`) for the
...
`<plugin-root>/references/agent-prompts/ce-web-researcher.md` does not exist (upstream rename,
