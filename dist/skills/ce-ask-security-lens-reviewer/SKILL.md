...
Dispatch the **ce-security-lens-reviewer** specialist by spawning a sub-agent with the
...
`references/agent-prompts/ce-security-lens-reviewer.md` **inside the plugin's install directory** (NOT the
...
1. Locate the prompt file via the discovery instructions above. Then read
   the resolved path (`<plugin-root>/references/agent-prompts/ce-security-lens-reviewer.md`) for the
...
2. Spawn a new agent with `subagent_type: "general-purpose"` (the harness's
   ...
   You are operating as the ce-security-lens-reviewer specialist. Your role is defined
...
`<plugin-root>/references/agent-prompts/ce-security-lens-reviewer.md` does not exist (upstream rename,
partial install), report:

> Persona `ce-security-lens-reviewer` not found in this plugin's manifest. The plugin
