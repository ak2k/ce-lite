...
from lib.claude_runner import RunResult, run_claude_p  # noqa: E402
...
def evaluate_case(case: Case, rep: int, mode: str, model: str, timeout: int) -> CaseResult:
    ...
    if case.accepted_layers == ["none"]:
        ...
        ce_lite_layers = {
            Layer.META_AGENT.value, Layer.META_SKILL.value,
            Layer.PANEL.value, Layer.WRAPPER.value,
        }
        passed = not any(
            l.value in ce_lite_layers for l in res.intermediate_layers
        )
    else:
        passed = actual_layer in case.accepted_layers
        if passed and case.accepted_personas:
            passed = actual_persona in case.accepted_personas if actual_persona else False
    ...
    return CaseResult(
        ...
        intermediate_layers=[l.value for l in res.intermediate_layers],
        error=res.error,
    )
...
def report(results: list[CaseResult], by_case_id: dict[str, Case]) -> int:
    ...
    for case_id in sorted(by_case):
        ...
        status_icon = "✅" if rep_passed == len(reps) else (
            "❌" if rep_passed == 0 else "⚠️ "
        )
        ...
        prompt_preview = textwrap.shorten(case.prompt.strip(), width=70, placeholder="…")
        print(f"  {status_icon} [{case_id}] {rep_str} → layer={','.join(layers_seen)}"
              + (f" persona={','.join(personas_seen)}" if personas_seen else ""))
        print(f"     prompt: {prompt_preview}")
        if rep_passed < len(reps):
            print(f"     expected: layer in {case.accepted_layers}"
                  + (f", persona in {case.accepted_personas}" if case.accepted_personas else ""))
        print()

    print("=" * 70)
    print(f"Total: {passed}/{total} pass ({rate:.0f}%) — {duration_total:.1f}s wall-clock")
    print("=" * 70)
...
def main() -> int:
    ...
    parser.add_argument("--results", default=None,
                        help="optional JSONL path to dump per-case results")
    ...
    print(f"Running {len(cases)} cases × {args.reps} reps "
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for i, fut in enumerate(as_completed(futures), 1):
            ...
            print(f"  [{i}/{len(futures)}] {tag} {r.case_id} rep={r.rep} "
